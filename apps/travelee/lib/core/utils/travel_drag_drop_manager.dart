...
class TravelDragDropManager {
  ...
  void handleDragAccept({
    ...
    dev.log(
        '원본 날짜=${TravelDateFormatter.formatDate(sourceDate)}, 대상 날짜=${TravelDateFormatter.formatDate(targetDate)}');
    dev.log(
        '원본 Day=$sourceDayNumber, 원본 국가=$sourceCountry, 원본 플래그=$sourceCountryFlag');
    ...
    try {
      ...
      // 국가 정보 확인 및 기록
      dev.log(
          '드래그 전 소스 국가 정보: ${sourceDayData?.countryName ?? "없음"}, ${sourceDayData?.flagEmoji ?? "없음"}');
      dev.log(
          '드래그 전 타겟 국가 정보: ${targetDayData?.countryName ?? "없음"}, ${targetDayData?.flagEmoji ?? "없음"}');

      // 2. 소스와 타겟의 상태 확인
      final bool isSourceEmpty =
          (sourceDayData == null || sourceDayData.countryName.isEmpty);
      final bool isTargetEmpty =
          (targetDayData == null || targetDayData.countryName.isEmpty);

      // 3. 대상 날짜의 day 번호 계산
      final targetDayNumber =
          _calculateDayNumber(currentTravel.startDate!, targetDate);

      // 4. 원본 및 대상 일정 분리
      final allSchedules = List<Schedule>.from(currentTravel.schedules);

      // 이동할 일정 목록 (드래그된 일정들)
      final schedulesToMove =
          allSchedules.where((s) => scheduleIds.contains(s.id)).toList();

      // 타겟에 있는 일정들
      final targetExistingSchedules = allSchedules
          .where((s) =>
      ...
      final remainingSourceSchedules = allSchedules
          .where((s) =>
              !scheduleIds.contains(s.id) &&
      ...
      List<Schedule> updatedSchedules =
          List<Schedule>.from(currentTravel.schedules);
      ...
      if (!isSourceEmpty && !isTargetEmpty) {
        ...
        if (sourceCountryName.isNotEmpty &&
            (sourceFlagEmoji.isEmpty || sourceFlagEmoji == "🏳️")) {
          // 국가 정보에서 이모지 찾기
          final countryInfo = currentTravel.countryInfos.firstWhere(
            (info) => info.name == sourceCountryName,
            orElse: () => CountryInfo(
                name: sourceCountryName, countryCode: '', flagEmoji: '🏳️'),
          );
          sourceFlagEmoji = countryInfo.flagEmoji;
          dev.log('소스 이모지 복원: $sourceCountryName -> $sourceFlagEmoji');
        }

        if (targetCountryName.isNotEmpty &&
            (targetFlagEmoji.isEmpty || targetFlagEmoji == "🏳️")) {
          // 국가 정보에서 이모지 찾기
          final countryInfo = currentTravel.countryInfos.firstWhere(
            (info) => info.name == targetCountryName,
            orElse: () => CountryInfo(
                name: targetCountryName, countryCode: '', flagEmoji: '🏳️'),
          );
        ...
        updatedDayDataMap[sourceDateKey] = sourceDayData.copyWith(
          ...
          schedules: targetExistingSchedules
                  .map((s) => s.copyWith(
                        date: sourceDate,
                        dayNumber: sourceDayNumber,
                      ))
                  .toList() +
              remainingSourceSchedules,
        );
      ...
      // 그 외 모든 경우 - 무조건 교환 방식으로 처리
        ...
        sourceCountryName = targetCountryName.isNotEmpty
            ? targetCountryName
            : (currentTravel.destination.isNotEmpty
      ...
      dev.log(
          '업데이트 완료 - 소스 국가 정보: ${updatedSourceDayData?.countryName ?? "없음"}, 코드: ${updatedSourceDayData?.countryCode ?? "없음"}');
      dev.log(
          '업데이트 완료 - 타겟 국가 정보: ${updatedTargetDayData?.countryName ?? "없음"}, 코드: ${updatedTargetDayData?.countryCode ?? "없음"}');
  ...
  void _verifyDragExchangeResult(
      String travelId, DateTime sourceDate, DateTime targetDate) {
    Future.delayed(const Duration(milliseconds: 100), () {
      ...
      if (sourceDayData != null) {
        dev.log(
            '  - 원본 Day ${sourceDayData.dayNumber}: 국가=${sourceDayData.countryName}, 일정=${sourceDayData.schedules.length}개');
      }
      if (targetDayData != null) {
        dev.log(
            '  - 대상 Day ${targetDayData.dayNumber}: 국가=${targetDayData.countryName}, 일정=${targetDayData.schedules.length}개');
      }

...
class TravelDialogManager {
  ...
  static Future<void> showEditTravelDialog(BuildContext context, WidgetRef ref) async {
    ...
    final result = await showDialog<Map<String, dynamic>>(
      ...
      builder: (context) => EditTravelDialog(
        ...
        initialEndDate: currentTravel.endDate ?? DateTime.now().add(const Duration(days: 1)),
      ),
    );
    ...
    if (result != null) {
      ...
      final newCountryInfos = (result['countryInfos'] as List).cast<CountryInfo>();
      
      // 삭제된 국가 확인
      final removedDestinations = oldDestinations.where((d) => 
        !newDestination.contains(d)
      ).toList();
      
      // 새로 추가된 국가 확인
      final addedDestinations = newDestination.where((d) => 
        !oldDestinations.contains(d)
      ).toList();
      ...
      if (removedDestinations.isNotEmpty) {
        ...
        _resetCountryInfoForRemovedDestinations(ref, currentTravel, removedDestinations, newDestination);
      }
      ...
      if (isDateChanged) {
        ...
        if (shouldProceed == true) {
          ...
          if (schedules.isNotEmpty) {
            final oldDateRange = DateUtil.getAllDates(currentTravel.startDate, currentTravel.endDate);
  ...
  static void _resetCountryInfoForRemovedDestinations(
    ...
    if (travel.dayDataMap.isEmpty && travel.startDate != null && travel.endDate != null) {

...
class _TravelDetailScreenState extends ConsumerState<TravelDetailScreen> with WidgetsBindingObserver {
  ...
  void _createBackup() {
    ...
    try {
      ...
      print('TravelDetailScreen - 백업할 여행 정보: ID=${travelInfo.id}, 일정=${travelInfo.schedules.length}개');
      ...
      for (int i = 0; i < Math.min(3, dayDataMap.length); i++) {
        ...
        print('TravelDetailScreen - 백업 데이터[${i}]: 날짜=${key}, 국가=${dayData?.countryName}, 플래그=${dayData?.flagEmoji}');
      }
  ...
  bool _detectChanges() {
    ...
    print('TravelDetailScreen - travelsProvider.hasChanges(): $providerHasChanges');
    ...
    if (_originalScheduleBackup.length != currentSchedules.length) {
      print('TravelDetailScreen - 일정 개수가 변경되었습니다 (${_originalScheduleBackup.length} -> ${currentSchedules.length})');
      return true;
    }
  ...
  Future<void> _restoreFromBackup() async {
    ...
    if (_originalScheduleBackup.isEmpty) {
      ...
      print('TravelDetailScreen - 현재 백업 데이터 크기: ${_originalScheduleBackup.length}');
      
      final currentTravel = ref.read(currentTravelProvider);
      if (currentTravel != null) {
        print('TravelDetailScreen - 현재 스케줄 수: ${currentTravel.schedules.length}');
      }
    } else {
      print('TravelDetailScreen - 백업 데이터 유효성 확인됨: ${_originalScheduleBackup.length}개 일정');
    ...
    try {
      ...
      print('TravelDetailScreen - dayDataMap 복원 준비 완료: ${dayDataMapCopy.length}개 날짜 데이터');
      ...
      if (currentTravel != null) {
        ...
        print('TravelDetailScreen - 복원된 여행 정보: ID=${updatedTravel.id}, 일정=${updatedTravel.schedules.length}개, 날짜 데이터=${updatedTravel.dayDataMap.length}개');
      ...
      print('TravelDetailScreen - 복원 후 일정 수: ${ref.read(currentTravelProvider)?.schedules.length ?? 0}개 (백업 원본: ${_originalScheduleBackup.length}개)');
  ...
  Widget build(BuildContext context) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (_backupCreated && mounted) {
        ...
        if (travelInfo != null && _originalTravelInfoBackup != null) {
          ...
          final isExistingTravel = travelInfo.id.isNotEmpty && !travelInfo.id.startsWith('temp_');
      ...
      return Scaffold(
        ...
        body: Column(
          children: [
            ...
            _buildBottomActionButton(context, updatedTravel),
          ],
        ),
      );
    }
    ...
    return PopScope(
      ...
      onPopInvoked: (didPop) async {
        ...
        print('TravelDetailScreen - 뒤로가기 감지: hasChanges=$hasChanges, isNewTravel=$isNewTravel');
          ...
          children: [
            ...
            Expanded(
              child: daySchedules.isEmpty
                  ...
                  : _buildDaySchedulesList(context, travelInfo, daySchedules, dragDropManager),
            ),
            _buildBottomActionButton(context, travelInfo),
          ],
        ),
      ),
    );
  ...
  void _initializeDaySchedules(dynamic travelInfo, List<DateTime> dates) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      // dayDataMap 업데이트
      final updatedDayDataMap = Map<String, DayScheduleData>.from(travelInfo.dayDataMap);
  ...
  void _verifyTravelIdMatch(dynamic travelInfo, List<DateTime> dates, List<dynamic> daySchedules) {
    final hasCorrectTravelId = daySchedules.any((ds) {
      final schedules = ds['schedules'] as List<dynamic>;
      return schedules.isEmpty || (schedules.isNotEmpty && schedules.first.travelId == travelInfo.id);
    });
    ...
    return AppBar(
      ...
      leading: IconButton(
        onPressed: () async {
          ...
          // 변경 사항이 있는지 확인
          final hasChanges = _detectChanges();
          print('TravelDetailScreen - 앱바 뒤로가기 버튼 클릭: hasChanges=$hasChanges, isNewTravel=$isNewTravel');
          ...
          if (hasChanges) {
            ...
            if (shouldSave == true) {
              ...
              await _restoreFromBackup();
              ref.read(currentTravelIdProvider.notifier).state = '';
              ref.read(currentTravelIdProvider.notifier).state = ref.read(currentTravelIdProvider);
              Navigator.pop(context);
            }
            // shouldSave가 null이면 (다이얼로그에서 취소 선택) 아무것도 하지 않음
          } else {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        if (travelInfo.dayDataMap.containsKey(dateKey)) {
          ...
          print('TravelDetailScreen - _buildDaySchedulesList: 날짜 $dateKey의 최신 데이터 국가=${latestDayData?.countryName ?? "없음"}, 국기=${latestDayData?.flagEmoji ?? "없음"}');
        } else {
          print('TravelDetailScreen - _buildDaySchedulesList: 날짜 $dateKey의 데이터 없음, 기본값 사용');
        }
                ...
                try {
                  ...
                  if (travelInfo.dayDataMap.containsKey(dateKey)) {
                    ...
                    if (dayData != null) {
                      ...
                      print('TravelDetailScreen - 삭제 전 국가 정보 백업: $deletedCountryName $deletedFlagEmoji');
                    }
                  }
                  ...
                  if (currentTravel != null) {
                    ...
                    if (deletedCountryName != null && deletedFlagEmoji != null) {
                      ...
                      print('TravelDetailScreen - 국가 정보 보존 완료: $dateKey - $deletedCountryName $deletedFlagEmoji');
                    }
                    ...
                    print('TravelDetailScreen - 업데이트된 여행 정보: 일정=${updatedSchedules.length}개, 날짜 데이터=${updatedDayDataMap.length}개');
                    ref.read(travelsProvider.notifier).updateTravel(updatedTravel);
                  }
  ...
  Widget _buildBottomActionButton(BuildContext context, dynamic travelInfo) {
    ...
    // 디버깅용 더 상세한 로그
    print('TravelDetailScreen - 버튼 로직: ID=${travelInfo.id}, isNewTravel=$isNewTravel');
      ...
      child: SizedBox(
        ...
        child: B2bButton.medium(
          ...
          onTap: () {
            ...
            if (isNewTravel) {
              print('TravelDetailScreen - 새 여행 저장 시작: 목적지=${travelInfo.destination.join(", ")}, 기간=${travelInfo.startDate} ~ ${travelInfo.endDate}');
              ...
              if (currentId.isNotEmpty && currentId.startsWith('temp_')) {
                ...
                final newId = ref.read(travelsProvider.notifier).saveTempTravel(currentId);
                
                if (newId != null) {
                  print('TravelDetailScreen - 임시 여행 ID 변경됨: $currentId -> $newId');
